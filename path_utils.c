...
bool paths_related(const char* path1, const char* path2) {
    if(strcmp(path1, path2) == 0) {
        return false;
    }
    int index = 0;

    while(path1[index] != '\0' && path2[index] != '\0') {
...
char *find_LCA(const char* path1, const char* path2) {

    if(strlen(path1) == 1 || strlen(path2) == 1 || path1[1] != path2[1]) {
        char *result = malloc((strlen(path1) + 1) * sizeof(char));
        if(!result) {
            fatal("Malloc failed!");
        }
        result[0] = '/';
    ...
    if(strlen(path1) < strlen(path2)) {
        shorterLength = strlen(path1);
    } else {
        shorterLength = strlen(path2);
    }

    char *result = malloc((shorterLength + 1) * sizeof(char));
    if(!result) {
        fatal("Malloc failed!");
    }
    size_t index = 0;
